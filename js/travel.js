...
var $ = require("jquery");
var page = {
  BMAP_AK: 'x21SV8yA2b10uiY6A13aqvuDEA3jp804',
  MAKERS: [],
  init: function () {
  ...
  loadConf: function loadConf() {
    return new Promise(function (resolve, reject) {
      $.getJSON('/assets/travel.json', function (response) {
        resolve(response);
      });
    });
  },
   ...
   * @returns
   */
  loadScript: function loadScript(ak) {
    return new Promise(function (resolve, reject) {
  ...
  mountMap: function mountMap() {
    ...
    page.loadScript(page.BMAP_AK).then(function (BMap) {
      var map = new BMap.Map('Bmap');
      var point = new BMap.Point(120.52, 30.45);
      ...
      if (address && address.length) {
        ...
        for (var i = 0; i < address.length; i++) {
          if (_typeof(address[i]) == 'object') {
            addPoint(map, address[i]);
          } else {
  ...
  setSwiperPhotos: function setSwiperPhotos() {
    var mySwiper = new Swiper('.my-photo-swiper', {
      slidesPerView: 'auto',
      spaceBetween: 10,
      autoplay: {
        delay: 0,
  ...
  loadEvent: function loadEvent() {
    ...
    PhotoModal.addEventListener('show.bs.modal', function (event) {
      ...
      var imgSrc = $(target).attr('data-bs-src');
      var modalImage = PhotoModal.querySelector('.madal-img');
      $(modalImage).attr('src', imgSrc);
    });
  },
  mountKeywords: function mountKeywords() {
    var keywords = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : [];
    for (var i = 0; i < keywords.length; i++) {
      $("#KeywordsList").append("<li>".concat(keywords[i], "</li>"));
    }
  }
};
page.init();
